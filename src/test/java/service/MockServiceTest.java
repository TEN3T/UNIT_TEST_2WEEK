...
public class MockServiceTest {
    ...
    //1-1. 어떤 포지션을 set 하더라도 무조건 미드로 리턴시키는 테스트
    @Test
    public void 무조건_미드_리턴(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 이름_가져오기_검증_테스트_2(){
        Champion topChampion = mock(Champion.class);
        topChampion.setName("조이");
        topChampion.setPosition("미드");
        topChampion.setHasSkinCount(3);

        System.out.println(topChampion.getName());
        System.out.println(topChampion.getName());
        System.out.println(topChampion.getName());


        verify(topChampion, atMost(3)).getName(); //최대 3번까지 OK
    }

    // 4-3. champion 객체에서 이름을 저장하는 로직이 실행되지 않았으면 Pass 하는 로직을 작성하세요.
    @Test
    public void 이름_저장_테스트() {
        Champion top = mock(Champion.class);

        verify(top, never()).setName(anyString());
    }

    // 4-4. champion 객체에서 이름을 가져오는 로직이 200ms 시간 이내에 1번 실행되었는 지 검증하는 로직을 작성하세요.
    @Test
    public void 이름_가져오기_테스트() {
        Champion champion = mock(Champion.class);
        champion.setName("조이");
        champion.setPosition("미드");
        champion.setHasSkinCount(3);
